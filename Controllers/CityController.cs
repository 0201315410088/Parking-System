...
public class CityController : Controller
{
    ...
    public CityController(ApplicationDbContext context, IOptions<StripeSettings> stripeSettings)
    {
        _context = context;
    ...
    public async Task<IActionResult> CampusRecords()
    {
        ...
        var bookings = await _context.CityNewBookings
                                     .Where(b => b.UserEmail == userEmail)
                                     .Include(b => b.Slot)
                                     .OrderByDescending(b => b.BookingTime)
                                     .ToListAsync();

        var bookingViewModels = bookings.Select(b => new CityCarImageBookingViewModel
        {
            CityBookingViewModel = new CityBookingViewModel
            {
    ...
    [HttpPost]
    public async Task<IActionResult> BookSlot(int slotId, DateTime bookingStartTime, DateTime bookingEndTime, CityCarType carType, string carPlate)
    {
        string userEmail = User.FindFirstValue(ClaimTypes.Email);
        if (string.IsNullOrEmpty(userEmail))
        {
        ...
        var slot = await _context.CityNewSlots.FirstOrDefaultAsync(s => s.SlotId == slotId);
        if (slot == null)
        {
        ...
        var existingBooking = await _context.CityNewBookings
        ...
        var isConflicted = await _context.CityNewBookings
        ...
        var duplicateCarPlate = await _context.CityNewBookings
        ...
        var booking = new CityNewBooking
        {
            SlotId = slotId,
            UserEmail = userEmail,
            CityCarType = carType,
        ...
        _context.CityNewBookings.Add(booking);
    ...
    public async Task<IActionResult> SignOutFromParking(int bookingId)
    {
        try
        {
            var booking = await _context.CityNewBookings.FindAsync(bookingId);
            if (booking == null)
            {
    ...
    public async Task<IActionResult> Success(int bookingId)
    {
        ...
        if (booking != null)
        {
            ...
            var transaction = new CityTransactionHistory
            {
            ...
            _context.CityTransactionHistorys.Add(transaction);

            // Remove the booking from NewBookings after payment is confirmed
            _context.CityNewBookings.Remove(booking);
    ...
    public async Task<IActionResult> PaymentHistory()
    {
        ...
        var transactionHistory = await _context.CityTransactionHistorys
            .OrderByDescending(t => t.StartTime)
            .ToListAsync();

        // Create a list of ProfileTransactionViewModel
        var profileTransactionViewModels = new List<CityProfileTransactionViewModel>();
        ...
        foreach (var transaction in transactionHistory)
        {
            ...
            profileTransactionViewModels.Add(new CityProfileTransactionViewModel
            {
                Profile = profile,
                CityTransactionHistory = transaction
            });
        }
    ...
    private double CalculateCost(TimeSpan duration, string email)
    {
        ...
        CityNewBooking objcar = new CityNewBooking(); // Pass the required email parameter

        double rate = 0;
        if (obj.UserType == UserType.Student)
        {
            if (objcar.CityCarType == CityCarType.Sedan)
            {
                rate = 13.17;
            }
            else if (objcar.CityCarType == CityCarType.SUV)
            {
                rate = 14.17;
            }
            else if (objcar.CityCarType == CityCarType.Hatchback)
            {
                rate = 13.50;
            }
            else if (objcar.CityCarType == CityCarType.Coupe)
            {
                rate = 14.00;
            }
            else if (objcar.CityCarType == CityCarType.Convertible)
            {
                rate = 14.50;
            }
            else if (objcar.CityCarType == CityCarType.Wagon)
            {
                rate = 13.75;
            }
            else if (objcar.CityCarType == CityCarType.Van)
            {
                rate = 14.25;
            }
            else if (objcar.CityCarType == CityCarType.Truck)
            {
                rate = 15.00;
            }
        }
    ...
    public async Task<IActionResult> CancelBooking(int bookingId)
    {
        try
        {
            var booking = await _context.CityNewBookings.FirstOrDefaultAsync(b => b.BookingId == bookingId);
            if (booking == null)
            {
...
public class CityCarImageController : Controller
{
    private readonly ApplicationDbContext _context;
    private readonly UserManager<IdentityUser> _userManager;

    public CityCarImageController(ApplicationDbContext context, UserManager<IdentityUser> userManager)
    {
        _context = context;
