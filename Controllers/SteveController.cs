...
public class SteveController : Controller
{
    ...
    public SteveController(ApplicationDbContext context, IOptions<StripeSettings> stripeSettings)
    {
        _context = context;
    ...
    public async Task<IActionResult> CampusRecords()
    {
        ...
        var bookings = await _context.SteveNewBookings
                                      .Include(b => b.Slot)
                                      .OrderByDescending(b => b.BookingTime)
                                      .ToListAsync();

        // Create a list of booking view models
        var bookingViewModels = bookings.Select(b => new SteveCarImageBookingViewModel
        {
            SteveBookingViewModel = new SteveBookingViewModel
            {
    ...
    [HttpPost]
    public async Task<IActionResult> BookSlot(int slotId, DateTime bookingStartTime, DateTime bookingEndTime, SteveCarType carType, string carPlate)
    {
        string userEmail = User.FindFirstValue(ClaimTypes.Email);
        if (string.IsNullOrEmpty(userEmail))
        {
        ...
        var slot = await _context.SteveNewSlots.FirstOrDefaultAsync(s => s.SlotId == slotId);
        if (slot == null)
        {
        ...
        var existingBooking = await _context.SteveNewBookings
        ...
        var isConflicted = await _context.SteveNewBookings
        ...
        var duplicateCarPlate = await _context.SteveNewBookings
    ...
    public async Task<IActionResult> SignOutFromParking(int bookingId)
    {
        try
        {
            var booking = await _context.SteveNewBookings.FindAsync(bookingId);
            if (booking == null)
            {
    ...
    public async Task<IActionResult> Success(int bookingId)
    {
        ...
        if (booking != null)
        {
            ...
            var transaction = new SteveTransactionHistory
            {
            ...
            _context.SteveTransactionHistorys.Add(transaction);

            // Remove the booking from NewBookings after payment is confirmed
            _context.SteveNewBookings.Remove(booking);
    ...
    public async Task<IActionResult> PaymentHistory()
    {
        ...
        var transactionHistory = await _context.SteveTransactionHistorys
            .OrderByDescending(t => t.StartTime)
            .ToListAsync();

        // Create a list of ProfileTransactionViewModel
        var profileTransactionViewModels = new List<SteveProfileTransactionViewModel>();
        ...
        foreach (var transaction in transactionHistory)
        {
            ...
            profileTransactionViewModels.Add(new SteveProfileTransactionViewModel
            {
                Profile = profile,
                SteveTransactionHistory = transaction
            });
        }
    ...
    private double CalculateCost(TimeSpan duration, string email)
    {
        ...
        SteveNewBooking objcar = new SteveNewBooking(); // Pass the required email parameter

        double rate = 0;
        if (obj.UserType == UserType.Student)
        {
            if (objcar.SteveCarType == SteveCarType.Sedan)
            {
                rate = 13.17;
            }
            else if (objcar.SteveCarType == SteveCarType.SUV)
            {
                rate = 14.17;
            }
            else if (objcar.SteveCarType == SteveCarType.Hatchback)
            {
                rate = 13.50;
            }
            else if (objcar.SteveCarType == SteveCarType.Coupe)
            {
                rate = 14.00;
            }
            else if (objcar.SteveCarType == SteveCarType.Convertible)
            {
                rate = 14.50;
            }
            else if (objcar.SteveCarType == SteveCarType.Wagon)
            {
                rate = 13.75;
            }
            else if (objcar.SteveCarType == SteveCarType.Van)
            {
                rate = 14.25;
            }
            else if (objcar.SteveCarType == SteveCarType.Truck)
            {
                rate = 15.00;
            }
        }
    ...
    public async Task<IActionResult> CancelBooking(int bookingId)
    {
        try
        {
            var booking = await _context.SteveNewBookings.FirstOrDefaultAsync(b => b.BookingId == bookingId);
            if (booking == null)
            {
...
public class SteveCarImageController : Controller
{
    private readonly ApplicationDbContext _context;
    private readonly UserManager<IdentityUser> _userManager;

    public SteveCarImageController(ApplicationDbContext context, UserManager<IdentityUser> userManager)
    {
        _context = context;
