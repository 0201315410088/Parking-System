...
public class RitsonController : Controller
{
    private readonly ApplicationDbContext _context;
    private readonly StripeSettings _stripeSettings;

    public RitsonController(ApplicationDbContext context, IOptions<StripeSettings> stripeSettings)
    {
        _context = context;
    ...
    public async Task<IActionResult> CampusRecords()
    {
        ...
        var bookings = await _context.RitsonNewBookings
                                      .Include(b => b.Slot)
                                      .OrderByDescending(b => b.BookingTime)
                                      .ToListAsync();

        // Create a list of booking view models
        var bookingViewModels = bookings.Select(b => new RitsonCarImageBookingViewModel
        {
            RitsonBookingViewModel = new RitsonBookingViewModel
            {
    ...
    [HttpPost]
    public async Task<IActionResult> BookSlot(int slotId, DateTime bookingStartTime, DateTime bookingEndTime, RitsonCarType carType, string carPlate)
    {
        string userEmail = User.FindFirstValue(ClaimTypes.Email);
        if (string.IsNullOrEmpty(userEmail))
        {
        ...
        var slot = await _context.RitsonNewSlots.FirstOrDefaultAsync(s => s.SlotId == slotId);
        if (slot == null)
        {
        ...
        var existingBooking = await _context.RitsonNewBookings
        ...
        var isConflicted = await _context.RitsonNewBookings
        ...
        var duplicateCarPlate = await _context.RitsonNewBookings
    ...
    public async Task<IActionResult> SignOutFromParking(int bookingId)
    {
        try
        {
            var booking = await _context.RitsonNewBookings.FindAsync(bookingId);
            if (booking == null)
            {
    ...
    public async Task<IActionResult> Success(int bookingId)
    {
        ...
        if (booking != null)
        {
            ...
            var transaction = new RitsonTransactionHistory
            {
            ...
            _context.RitsonTransactionHistorys.Add(transaction);

            // Remove the booking from NewBookings after payment is confirmed
            _context.RitsonNewBookings.Remove(booking);
    ...
    public async Task<IActionResult> PaymentHistory()
    {
        ...
        var transactionHistory = await _context.RitsonTransactionHistorys
            .OrderByDescending(t => t.StartTime)
            .ToListAsync();

        // Create a list of ProfileTransactionViewModel
        var profileTransactionViewModels = new List<RitsonProfileTransactionViewModel>();
        ...
        foreach (var transaction in transactionHistory)
        {
            ...
            profileTransactionViewModels.Add(new RitsonProfileTransactionViewModel
            {
                Profile = profile,
                RitsonTransactionHistory = transaction
            });
        }
    ...
    private double CalculateCost(TimeSpan duration, string email)
    {
        ...
        RitsonNewBooking objcar = new RitsonNewBooking(); // Pass the required email parameter

        double rate = 0;
        if (obj.UserType == UserType.Student)
        {
            if (objcar.RitsonCarType == RitsonCarType.Sedan)
            {
                rate = 13.17;
            }
            else if (objcar.RitsonCarType == RitsonCarType.SUV)
            {
                rate = 14.17;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Hatchback)
            {
                rate = 13.50;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Coupe)
            {
                rate = 14.00;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Convertible)
            {
                rate = 14.50;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Wagon)
            {
                rate = 13.75;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Van)
            {
                rate = 14.25;
            }
            else if (objcar.RitsonCarType == RitsonCarType.Truck)
            {
                rate = 15.00;
            }
        }
    ...
    public async Task<IActionResult> CancelBooking(int bookingId)
    {
        try
        {
            var booking = await _context.RitsonNewBookings.FirstOrDefaultAsync(b => b.BookingId == bookingId);
            if (booking == null)
            {
...
public class RitsonCarImageController : Controller
{
    private readonly ApplicationDbContext _context;
    private readonly UserManager<IdentityUser> _userManager;

    public RitsonCarImageController(ApplicationDbContext context, UserManager<IdentityUser> userManager)
    {
        _context = context;
